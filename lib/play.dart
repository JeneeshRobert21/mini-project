...
import 'package:audiobooktest/bookmark.dart';
...
//     print(response.data); // "ASSEMBLY_COMPLETED"
  ...
  void initState() {
    ...
    initPlayer();
    final bookmarksService =
        BookmarksService(userId: widget.user, bookId: widget.book.bookId);
    bookmarksService.setPageNumber(widget.pageNumber);
  }
  ...
  void dispose() {
    ...
    final bookmarksService =
        BookmarksService(userId: widget.user, bookId: widget.book.bookId);
    bookmarksService.incrementPageNumber();
    super.dispose();
  }
